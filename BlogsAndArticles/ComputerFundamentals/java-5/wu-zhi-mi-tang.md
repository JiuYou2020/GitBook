...
> 推荐阅读：[https://blog.csdn.net/qq\_41907991/article/details/107164508](https://blog.csdn.net/qq\_41907991/article/details/107164508)
...
`aspect` 由 `pointcount` 和 `advice` 组成, 它既包含了横切逻辑的定义, 也包括了连接点的定义. ==Spring AOP就是负责实施切面的框架, 它将切面(Aspect)所定义的横切逻辑(advice)织入到切面所指定(point cut)的连接点(join point)中.== AOP的工作重心在于如何将增强织入目标对象的连接点上, 这里包含两个工作:
...
2. 隔离级别
   * **TransactionDefinition.ISOLATION\_DEFAULT:** 使用后端数据库默认的隔离级别，Mysql 默认采用的 **REPEATABLE\_READ**隔离级别 Oracle 默认采用的 READ\_COMMITTED隔离级别.
...
详见[Cache与DB的数据一致性](../java-2/cache-yu-db-de-shu-ju-yi-zhi-xing.md)
...
两者存储`hello`所占的空间一样，**但是后者在排序时会消耗更多内存，因为`ORDER BY col` 采用 fixed\_length 计算 col 长度(memory引擎也一样)。**
...
5. 每个非叶子节点包含 $n$ 个关键字信息（$P\_0,P\_1,…P\_n, k\_1,…k\_n$）。
6. 关键字的个数 $n$ 满足：$\lceil m/2 \rceil - 1 \leq n \leq m-1$。
7. $k\_i (i=1,…n)$ 为关键字，且关键字升序排序。
8. $P\_i (i=0,…n)$ 为指向子树根节点的指针。$P\_{i-1}$ 指向的子树的所有节点关键字均小于 $k\_i$，但都大于 $k\_{i-1}$。
...
生产事故：请参考[https://cloud.tencent.com/developer/article/1801978](https://cloud.tencent.com/developer/article/1801978)
...
1. **隐藏字段**：在内部，`InnoDB` 存储引擎为每行数据添加了三个 [隐藏字段](https://dev.mysql.com/doc/refman/5.7/en/innodb-multi-versioning.html)：
...
2.  **ReadView**:[`Read View`](https://github.com/facebook/mysql-8.0/blob/8.0/storage/innobase/include/read0types.h#L298) 主要是用来做可见性判断，里面保存了 “当前对本事务不可见的其他活跃事务”
    ...
    * `m_creator_trx_id`：**创建该 `Read View` 的事务 ID**
    ...
    ![trans\_visible](../.gitbook/assets/trans\_visible-ekj9bMvL.png)
...
3.  **undo-log**:`undo log` 主要有两个作用：
    ...
    **在 `InnoDB` 存储引擎中 `undo log` 分为两种：`insert undo log` 和 `update undo log`：**
    ...
    2.  **`update undo log`**：`update` 或 `delete` 操作中产生的 `undo log`。该 `undo log`可能需要提供 `MVCC` 机制，因此不能在事务提交时就进行删除。提交时放入 `undo log` 链表，等待 `purge线程` 进行最后的删除。
        ...
        ![img](../.gitbook/assets/6a276e7a-b0da-4c7b-bdf7-c0c7b7b3b31c-n52toho\_.png)
...
以sync\_binlog=1为例，事务执行过程中，会先把日志写入binlog cache中，事务提交时才会把binlog cache中的日志持久化到磁盘的binlog文件中。可以通过控制binlog\_cache\_size参数控制单个线程binlog cache的大小，如果存储的内容超过这个值，就要暂存到磁盘（Swap）
...
* **持久性**：一旦事务提交，数据库系统必须确保这些事务的所有修改都永久保存，即使系统发生崩溃。
* **数据一致性**：数据库在任何时候都必须处于一致状态，即数据库的状态必须遵循事务的ACID（Atomicity、Consistency、Isolation、Durability）特性。

**Redo Log的原理**

* **事务日志的持久性**：在事务提交时，MySQL首先确保事务的Redo log记录已经写入磁盘，然后才会认为事务已提交。这是通过`fsync()`操作来保证的。
* **WAL（Write-Ahead Logging）协议**：WAL协议要求在数据页（实际数据）写入磁盘之前，必须先将其对应的Redo log记录写入磁盘。这确保了即使在数据页写入之前系统崩溃，也可以通过Redo log进行恢复。

**Redo Log的写入机制**

* **生成Redo Log记录**：当事务对数据进行修改时，首先会生成Redo log记录。这些记录存储在Redo log缓冲区中。
* **写入Redo Log缓冲区**：Redo log记录首先写入内存中的Redo log缓冲区。缓冲区中的日志记录会周期性地或在特定条件下（如事务提交）被写入磁盘上的Redo log文件。
* **事务提交时的刷盘**：当事务提交时，MySQL会将该事务的Redo log记录从缓冲区刷盘到Redo log文件中。此时会调用`fsync()`，确保日志记录被持久化到磁盘。
* **周期性刷盘**：MySQL会周期性地将Redo log缓冲区的内容刷盘到磁盘，即使没有事务提交。这可以通过参数`innodb_flush_log_at_trx_commit`进行控制。
...
即使在Redo log还未完全刷盘时宕机，MySQL可以通过以下机制来尽量减少数据丢失：

1. **双写缓冲（Doublewrite Buffer）**：
   * InnoDB使用双写缓冲技术，即将数据页先写入一个中间区域（双写缓冲区），然后再写入最终的存储位置。这样可以减少因崩溃导致的数据损坏风险。
2. **恢复过程**：
   * 在系统重新启动时，MySQL会从Redo log文件中读取未完成的事务记录，并重新应用这些记录以恢复数据到一致状态。这依赖于Redo log的完整性，因此确保Redo log的刷盘是至关重要的。
...
设计表时，一般会有一个自增主键。InnoDB中的自增主键（或其他auto\_increment字段）会涉及到自增锁（表级锁）。
...
* **分布式锁**：通过Redis做分布式锁是一种比较常见的方式。通常情况下，我们都是基于Redisson来实现分布式锁。详情看：[分布式锁详解](https://blog.csdn.net/weixin\_45150104/article/details/125131846)
...
* 通过 bio\_close\_file 后台线程来释放 AOF / RDB 等过程中产生的临时文件资源
* 通过 bio\_aof\_fsync 后台线程调用 fsync 函数将系统内核缓冲区还未同步到到磁盘的数据强制刷到磁盘（ AOF 文件）
* 通过 bio\_lazy\_free后台线程释放大对象（已删除）占用的内存空间
...
通常情况下，我们认为 mem\_fragmentation\_ratio > 1.5 的话才需要清理内存碎片。
...
1. 在读取一个键之后（读操作和写操作都要对键进行读取），服务器会根据键是否存在来更新服务器的键空间命中（hit）次数或键空间不命中（miss）次数，这两个值可以在INFO stats命令的keyspace\_hits属性和keyspace\_misses属性中查看。
...
* \*\*IK Analyzer：\*\*最常用，分为：
*
  * ik\_max\_word：细粒度切分，会做最细粒度拆分，尽可能拆分出更多单词
  * ik\_smart：智能模式，做粗粒度拆分，已被分出的词语不会再被其他词语占有
* **其他**
...
* 关键词：keyword、constant\_keyword、wildcard
* 数值型：long、integer、short、byte、double、float、half\_float、scaled\_float
* 布尔型：boolean
* 日期型：data、data\_nanos
* 二进制：binary

**结构化数据类型：**

* 范围型：integer\_range、float\_range、long\_range、double\_range、data\_range
* ip地址类型：ip
* 软件版本：version
...
keyword不走分词器，text会走，keyword效率更高，一般在fields中定义keyword类型字段：[https://netcut.cn/p/7c5238151898340e](https://netcut.cn/p/7c5238151898340e)
...
Nested数据类型可以避免**数组扁平化处理**（多个数组的字段做笛卡尔积）。[https://netcut.cn/p/1bae8387f079d30a](https://netcut.cn/p/1bae8387f079d30a)
...
使用`copy_to`，比如把first\_name和last\_name合并为full\_name，但full\_name不在查询结果中展示：
...
[https://netcut.cn/p/e5daf6ac18bbc3fd](https://netcut.cn/p/e5daf6ac18bbc3fd)

### 想要某个字段不被索引怎么做？

在Mapping中设置属性index=false，则该字段不可作为检索条件，但结果中仍包含该字段。

另外，还有一个属性index\_operations可以控制倒排索引记录内容，属性有：
...
1.  全量同步即建好ES索引后一次性导入MySQL的所有数据。
    ...
    go-mysql-elasticsearch是一项将MySQL数据同步到ES中的服务，同样支持增量同分布：[https://github.com/go-mysql-org/go-mysql-elasticsearch](https://github.com/go-mysql-org/go-mysql-elasticsearch)
...
* 加大 index\_buffer\_size ，这个是 ES 活跃分片共享的内存区，官方建议每个分片至少 512MB，且为 JVM 内存的 10%。
